...
const [timestamp, setTimestamp] = useState(new Date().toISOString());
  ...
  const handleAddItem = async (category) => {
    ...
    setSnackbarMessage(`Added ${newItems[category]} to ${category} successfully!`);
  ...
  const handleDeleteItems = async (category) => {
    ...
    if (selectedIds.length > 0) {
      try {
        ...
        setDropdownValues(prev => ({
          ...prev,
          [category]: prev[category].filter(item => !selectedIds.includes(item.id))
        }));
        setSelectedItems(prev => ({ ...prev, [category]: [] }));
        console.log(`Deleted ${selectedIds.length} items from ${category}`);
        setSnackbarMessage(`Deleted ${selectedIds.length} items from ${category} successfully!`);
      setOpenSnackbar(true);
      setSnackbarSeverity("success");
      } catch (error) {
          ...
          <Box sx={{ height: '100%' }}>
            ...
            {items.map((item, index) => (
              ...
              <div className="flex space-x-2">
                ...
                />
             <Button  color ="secondary" variant="contained"  onClick={() => handleAddItem(category)}>
                  <Plus className="mr-2 h-4 w-4 " /> Add
                </Button>
                <Button  variant="contained" color ="error"   onClick={() => handleDeleteItems(category)}  >
